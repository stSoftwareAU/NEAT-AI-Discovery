...
pub fn noise_signal_threshold(default: f32) -> f32 {
    parse_env("NEAT_AI_DISCOVERY_NOISE_SIGNAL_THRESHOLD").unwrap_or(default)
}
...
pub fn dominance_threshold(default: f32) -> f32 {
    parse_env("NEAT_AI_DISCOVERY_DOMINANCE_THRESHOLD").unwrap_or(default)
}
...
pub fn gradient_threshold(default: f32) -> f32 {
    parse_env("NEAT_AI_DISCOVERY_GRADIENT_THRESHOLD").unwrap_or(default)
}
...
pub fn module_tiering_hidden_neuron_threshold() -> usize {
    parse_env("NEAT_AI_DISCOVERY_MODULE_TIERING_HIDDEN_THRESHOLD")
...
pub fn target_cooldown_consecutive_failures_env() -> Option<u32> {
    parse_env("NEAT_AI_DISCOVERY_TARGET_COOLDOWN_FAILURES").filter(|v| *v >= 1)
}
...
pub fn target_cooldown_epochs_env() -> Option<u64> {
    parse_env("NEAT_AI_DISCOVERY_TARGET_COOLDOWN_EPOCHS").filter(|v| *v >= 1)
}
...
pub fn within_batch_target_failure_limit_env() -> Option<u32> {
    parse_env("NEAT_AI_DISCOVERY_BATCH_TARGET_FAILURE_LIMIT").filter(|v| *v >= 1)
}
