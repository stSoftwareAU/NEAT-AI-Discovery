...
use crate::analysis::shared::{AnalyzeNeuronsResult, AnalyzeSynapsesResult, TimingScope};

// Import activation functions from the dedicated activation module (Issue #266)
use crate::analysis::activation::{
    absolute_activation, activation_name_to_gpu_id, arctan_activation, bent_identity_activation,
    bipolar_activation, clipped_activation, elu_activation, gelu_activation, get_bias_range,
    get_bias_values, hard_tanh_activation, identity_activation, is_threshold_activation,
    logistic_activation, mish_activation, relu6_activation, softplus_activation,
    softsign_activation, tanh_activation, ActivationCandidateSpec, ACTIVATION_SPECS,
};

// Import memory and platform utilities from dedicated modules (Issue #267)
use crate::analysis::utils::{
    cap_gpu_batch_size_by_bytes, check_memory_for_parquet, check_system_memory_requirements,
    detect_memory_tier, ensure_xdg_runtime_dir, get_memory_info, get_work_queue_capacity,
    suppress_mesa_warnings_if_requested, verbose_enabled, MemoryTier, DEFAULT_GPU_BATCH_SIZE,
    HIGH_PERF_GPU_BATCH_SIZE, LOW_MEMORY_GPU_BATCH_SIZE,
};
...
// Import test-specific types from diagnostics and shared
#[cfg(test)]
use crate::analysis::diagnostics::RejectionReason;
#[cfg(test)]
...
// TargetData, TargetMap moved to crate::analysis::diagnostics module (Issue #271)
...
// FocusTargetFilterResult, filter_focus_targets_for_neuron_analysis
// moved to crate::analysis::diagnostics module (Issue #271)
...
#[cfg(test)]
mod tests {
    // Unit tests live under `tests/` where possible, but remain unit tests so they can
    // access private helpers without changing visibility.
    include!(concat!(
