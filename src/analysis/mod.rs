...
pub fn analyze_all(input: &AnalyzeAllInput) -> Result<AnalyzeAllResult> {
    ...
    // Issue #375: Dispatch all discovery detection modules using the generic pattern.
    // Each module encapsulates its own record collection, detection, and candidate
    // conversion logic. The dispatch function handles watchdog beats, phase timing,
    // verbose logging, and merging into the synapse result.
    if let Some(syn) = synapse_result.as_mut() {
        let modules = discovery_dispatch::modules::all_discovery_modules();
        let module_refs: Vec<&dyn discovery_dispatch::DiscoveryModule> =
            modules.iter().map(|m| m.as_ref()).collect();

        discovery_dispatch::dispatch_all_discovery_modules(
            &module_refs,
            &input.creature,
            &shared_cache,
            syn,
