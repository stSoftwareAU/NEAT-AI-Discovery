...
/// Discard add-synapse candidates whose `expected_creature_score_gain` is not
/// finite, then rank the survivors by gain (highest first). Returns the number
/// of candidates rejected.
///
/// Issue #2167: the ranking comparator is `f32::total_cmp`, whose IEEE-754
/// totalOrder places every finite gain below `+∞` and `+∞` below a positive
/// `NaN`, so a descending sort promotes a non-finite gain to position 0 and
/// displaces every genuine candidate. No upstream filter stops it — the
/// comparison screens (`gain >= floor` in
/// [`apply_min_expected_gain_floor_for_synapses`], `gain > 0.0` for the
/// coordinated list) are false for `NaN` but true for `+∞`. Gating immediately
/// before the sort is therefore the only placement that holds for every list.
///
/// Rejections are counted rather than dropped silently, matching the Issue
/// #1367 gate in
/// [`reject_non_finite_gains`](crate::analysis::candidate_aggregation::reject_non_finite_gains);
/// the total is surfaced under `REJECTION_NON_FINITE_GAIN` in the analysis
/// metadata so a poisoned batch is distinguishable from a genuine drought.
pub fn reject_non_finite_and_rank_synapse_candidates(
    candidates: &mut Vec<CandidateSynapseJson>,
) -> u32 {
    let before = candidates.len();
    candidates.retain(|c| c.expected_creature_score_gain.is_finite());
    let rejected = u32::try_from(before.saturating_sub(candidates.len())).unwrap_or(u32::MAX);
...
/// Coordinated-structural counterpart of
/// [`reject_non_finite_and_rank_synapse_candidates`] (Issue #2167), delegating
/// the rejection to the shared Issue #1367 gate. Returns the number of
/// candidates rejected.
pub fn reject_non_finite_and_rank_coordinated_candidates(
    candidates: &mut Vec<crate::CoordinatedStructuralCandidateJson>,
) -> u32 {
    let rejected = crate::analysis::candidate_aggregation::reject_non_finite_gains(candidates);
    candidates.sort_by(|a, b| {
        b.expected_creature_score_gain
            .total_cmp(&a.expected_creature_score_gain)
    });
...
pub fn scale_by_error_fraction(
    ...
    // Issue #2167: reject non-finite inputs explicitly. `NaN <= EPSILON` is
    // false, so the floor guard below passes a `NaN` total straight through to
    // the division, and `f32::clamp` propagates the `NaN` into the returned
    // gain — where it becomes a candidate gain the ranking sort cannot order.
    if !raw_prediction.is_finite() || !target_error_sq.is_finite() || !total_error_sq.is_finite() {
        return 0.0;
    }
...
fn compute_neuron_error_sq_map<'a>(
    ...
    for neuron in &input.creature.neurons {
        if let Ok(records) = cache.get(&neuron.uuid) {
            // Issue #2167: a finite error still squares to `+∞` once it exceeds
            // `√f32::MAX` (~1.84e19), so the input filter alone does not
            // guarantee a finite square — screen the square itself, and the
            // accumulated total, before it can reach `scale_by_error_fraction`.
            let error_sq: f32 = records
                .iter()
                .flat_map(|r| r.errors.iter())
                .filter(|e| e.is_finite())
                .map(|e| e * e)
                .filter(|sq| sq.is_finite())
                .sum();
            if error_sq.is_finite() && error_sq > EPSILON {
...
pub(crate) fn apply_post_processing(
    ...
    // Issue #2167: discard non-finite gains, then sort all candidate lists by
    // expected_creature_score_gain (highest first). The gate must sit here, not
    // in the upstream screens: those are comparisons, which reject `NaN` but
...
pub(crate) struct PostProcessingMetrics {
    ...
    /// Issue #2167: count of candidates discarded across all three lists
    /// because their expected gain was not finite.
    pub non_finite_gain_drops: u32,
}
...
pub(crate) struct MetadataParams<'a> {
    ...
    /// Issue #2167: count of candidates discarded before ranking because their
    /// expected gain was not finite.
    pub non_finite_gain_drops: u32,
}
...
pub(crate) fn build_metadata(
    ...
    crate::analysis::shared::SynapseAnalysisMetadata {
        ...
        rejection_breakdown: {
            ...
            // Issue #2167: record candidates discarded before ranking for a
            // non-finite expected gain, so a poisoned batch is distinguishable
            // from a genuine drought rather than vanishing silently.
            b.record_many_u32(
                crate::analysis::diagnostics::rejection_reasons::REJECTION_NON_FINITE_GAIN,
                params.non_finite_gain_drops,
            );
            b
        },
