//! Contract tests for the chunk 8b `synapse post-processing` sweep (Issue
//! #2105).
//!
//! `tests/issue_2103_chunk_08b_ledger_scaffold.rs` gates the record's *shape*;
//! `tests/issue_2104_chunk_08b_synapse_pipeline_sweep.rs` gates the sibling
...
//!   `max_by` / `min_by`) in the production half of those files is cited in
//!   the matching finding table, so a new one cannot land unrecorded;
//! * the symbols the outcome traces still exist;
//! * the four findings the sweep filed (#2167-#2170) are linked from both the
//!   outcome section and `## Issues filed`.
...
fn post_processing_rows() -> Vec<(String, String)> {
    let doc = read(RECORD);
    file_rows(section(&doc, "### synapse post-processing"))
}
...
#[test]
fn the_synapse_post_processing_section_owns_exactly_the_files_issue_2105_swept() {
    let rows = post_processing_rows();
    let paths: Vec<&str> = rows.iter().map(|(path, _)| path.as_str()).collect();
    assert_eq!(
        paths, POST_PROCESSING_FILES,
        "the `synapse post-processing` section must carry one row per file Issue #2105 swept, \
         in record order — a row under the wrong section has no owner and is swept twice or \
         never"
    );
}

#[test]
fn every_synapse_post_processing_row_is_swept_with_a_reason() {
    for (path, outcome) in post_processing_rows() {
        assert!(
            !outcome.contains("pending"),
            "{path} was swept by Issue #2105, so its outcome must not read `pending`: {outcome}"
        );
...
const TRACED_SYMBOLS: [(&str, &str); 11] = [
    ...
    (
        "src/analysis/synapse/structural_patterns.rs",
        "fn detect_noisy_vs_trusted",
    ),
    (
        "src/analysis/synapse/structural_patterns.rs",
        "fn detect_collapsible_hidden_neurons",
    ),
    (
        "src/analysis/synapse/adaptive_proposal.rs",
...
#[test]
fn the_synapse_post_processing_outcome_cites_symbols_that_still_exist() {
    let doc = read(RECORD);
    let outcome = section(&doc, "### synapse post-processing (Issue #2105)");
    ...
    for (file, declaration) in TRACED_SYMBOLS {
        ...
        assert!(
            ...
            "the `synapse post-processing` outcome must name `{symbol}` — an outcome that cites \
             no guard, bound or dispatch site is a claim, not a sweep"
        );
        assert!(
...
#[test]
fn the_synapse_post_processing_outcome_links_its_filed_findings() {
    let doc = read(RECORD);
    let outcome = section(&doc, "### synapse post-processing (Issue #2105)");
    let filed = section(&doc, "## Issues filed");

    for finding in ["#2167", "#2168", "#2169", "#2170"] {
        assert!(
