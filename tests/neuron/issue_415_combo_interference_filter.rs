//! Issue #415: combo-successful discovery must not propose interfering pairs.
//!
//! End-to-end coverage of the live interference filter:
//! `filter_interfering_epistatic_pairs` and
//! `filter_interfering_synergistic_candidates`, called from
//! `candidate_selection::detect_epistatic_and_synergistic`. Redundant
//! (highly correlated) inputs must not be offered as a coordinated pair, and
//! complementary inputs still must be.
//!
//! The unit tests of the never-called `detect_interfering_pairs` that used to
//! live here were deleted with that function (Issue #2192).
