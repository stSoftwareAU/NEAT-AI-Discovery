//! Contract tests for the chunk 8b `synapse pipeline` sweep (Issue #2104).
//!
...
const PIPELINE_FILES: [&str; 15] = [
    ...
    "src/analysis/synapse/candidate_generation.rs",
    "src/analysis/synapse/cpu_pre_reject.rs",
    ...
    "src/analysis/synapse/filtering.rs",
    "src/analysis/synapse/holdout_validation.rs",
...
fn pipeline_rows() -> Vec<(String, String)> {
    let doc = read(RECORD);
    file_rows(section(&doc, "### synapse pipeline"))
}
...
#[test]
fn the_synapse_pipeline_section_owns_exactly_the_files_issue_2104_swept() {
    let rows = pipeline_rows();
    let paths: Vec<&str> = rows.iter().map(|(path, _)| path.as_str()).collect();
    assert_eq!(
        paths, PIPELINE_FILES,
        "the `synapse pipeline` section must carry one row per file Issue #2104 swept, in \
         record order — a row under the wrong section has no owner and is swept twice or \
         never"
    );
}

#[test]
fn every_synapse_pipeline_row_is_swept_with_a_reason() {
    for (path, outcome) in pipeline_rows() {
        assert!(
            !outcome.contains("pending"),
            "{path} was swept by Issue #2104, so its outcome must not read `pending`: {outcome}"
        );
...
/// Every symbol the `synapse pipeline` outcome claims to have traced, paired
...
const TRACED_SYMBOLS: [(&str, &str); 10] = [
    (
        "src/analysis/synapse/candidate_generation.rs",
        "fn group_sources_by_locality",
    ),
    (
        "src/analysis/synapse/candidate_generation.rs",
        "fn compute_obs_index_overlap",
    ),
    (
        "src/analysis/synapse/candidate_generation.rs",
        "fn build_ordered_neurons",
    ),
    (
        "src/analysis/synapse/preparation.rs",
        "fn build_creature_lookups",
    ),
    (
        "src/analysis/synapse/preparation.rs",
        "fn compute_constant_source_threshold_from_cache",
    ),
    (
        "src/analysis/synapse/holdout_validation.rs",
        "fn split_samples_holdout",
    ),
    (
        "src/analysis/synapse/holdout_validation.rs",
        "fn is_validation_sample",
    ),
    (
        "src/analysis/synapse/scoring/improvement.rs",
        "fn finalise_improvement",
    ),
    (
        "src/analysis/synapse/scoring/improvement.rs",
        "fn select_finite",
    ...
    (
        "src/analysis/utils/deadline.rs",
        "pub fn apply_source_budget",
    ),
];

#[test]
fn the_synapse_pipeline_outcome_cites_symbols_that_still_exist() {
    let doc = read(RECORD);
    let outcome = section(&doc, "### synapse pipeline (Issue #2104)");
    ...
    for (file, declaration) in TRACED_SYMBOLS {
        ...
        assert!(
            ...
            "the `synapse pipeline` outcome must name `{symbol}` — an outcome that cites no \
...
#[test]
fn the_synapse_pipeline_outcome_links_its_filed_finding() {
    let doc = read(RECORD);
    let outcome = section(&doc, "### synapse pipeline (Issue #2104)");
    assert!(
        outcome.contains("#2161"),
        "the outcome must link the finding it filed — an unlinked finding is invisible to the \
