//! Shipped source, benches, and examples must not name private repositories
//! (Issue #1724).
//!
...
//! repository name reappears in shipped source.
//!
//! Test comments are covered separately by Issue #1725, so `tests/` is
//! deliberately outside this walk.

use std::path::{Path, PathBuf};

/// Directories whose Rust sources ship to, or are read by, the public.
const SCANNED_DIRS: [&str; 3] = ["src", "benches", "examples"];
...
fn snapshot_example_documents_public_reproducible_inputs() {
    ...
    let private_checkout = format!("{}{}", "~/src/", "GRQ");
    assert!(
