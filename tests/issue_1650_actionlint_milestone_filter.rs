//! Issue #1650: the actionlint CI quality workflow (`actionlint.yml`)
//! declared a `pull_request: branches: ["*"]` filter. In GitHub Actions
//! branch filters the `*` glob does not cross `/`, so `"*"` matches
//! single-level branch names (e.g. `Develop`) but never `milestone/<slug>`.
//! Milestone sub-issue PRs target a shared `milestone/<name>` branch, so the
//! actionlint gate never ran on those PRs and workflow regressions merged
//! into the milestone branch unchecked.
//!
//! The fix adds `milestone/*` alongside the existing glob so milestone PRs
//! are gated too. This test parses `actionlint.yml` as plain text (no YAML
...
#[test]
fn actionlint_pull_request_filter_matches_milestone_branches() {
    let body = read_workflow("actionlint.yml");
    let branches = pull_request_branches(&body)
        .expect("actionlint.yml must declare a `pull_request:` `branches:` filter");
    assert!(
        branches.contains("milestone/*"),
        "actionlint.yml `pull_request` branch filter must include \
         `milestone/*` so milestone sub-issue PRs are actionlint-gated \
         (Issue #1650). Found: {branches}",
    );
}

#[test]
fn actionlint_pull_request_filter_keeps_wildcard() {
    ...
    assert!(
        ...
        "actionlint.yml must keep the single-level `\"*\"` glob (or `\"**\"`) \
         so non-milestone PRs stay gated (Issue #1650). Found: {branches}",
    );
}
