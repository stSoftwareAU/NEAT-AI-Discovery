//! Issue #1648: the `semgrep` job's `actions/checkout` step in
...
//! a compromised dependency — can read it and act as the token. The `semgrep`
//! job only runs a SAST scan over the tree; it never pushes back to the
...
fn semgrep_checkout_disables_credential_persistence() {
    ...
    let block =
        job_block(&body, "semgrep").expect("`semgrep` job not found in semgrep.yml (Issue #1648)");

    assert!(
        block.contains("uses: actions/checkout@"),
        "`semgrep` job should still check out the repository (Issue #1648)",
    );
    assert!(
        block.contains("persist-credentials: false"),
        "`semgrep` job's checkout step must set `persist-credentials: false` \
         so the GITHUB_TOKEN is not written to disk (Issue #1648)",
    );
}
