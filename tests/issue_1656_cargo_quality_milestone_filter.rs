//! Issue #1656: the Coverage workflow (`cargo-quality.yml`) declared a
...
//! sub-issue PRs target a shared `milestone/<name>` branch, so the
//! coverage gate never ran on those PRs and coverage regressions
//! accumulated on the milestone branch unobserved.
//!
//! The fix adds `milestone/*` alongside the existing glob so milestone PRs
//! are gated too. This test parses `cargo-quality.yml` as plain text (no
//! YAML parser is in the dependency tree, matching the sibling workflow
//! tests) and asserts the `pull_request:` branch filter both keeps the
//! original `"*"` glob and includes `milestone/*`.
...
#[test]
fn cargo_quality_pull_request_filter_matches_milestone_branches() {
    let body = read_workflow("cargo-quality.yml");
    let branches = pull_request_branches(&body)
        .expect("cargo-quality.yml must declare a `pull_request:` `branches:` filter");
    assert!(
        branches.contains("milestone/*"),
        "cargo-quality.yml `pull_request` branch filter must include \
         `milestone/*` so milestone sub-issue PRs are coverage-gated \
         (Issue #1656). Found: {branches}",
    );
}

#[test]
fn cargo_quality_pull_request_filter_keeps_wildcard() {
    ...
    assert!(
        ...
        "cargo-quality.yml must keep the single-level `\"*\"` glob (or `\"**\"`) \
         so non-milestone PRs stay gated (Issue #1656). Found: {branches}",
    );
}
