...
| `NEAT_AI_DISCOVERY_GPU` | `auto` | Host GPU policy (GRQ#4405). `off` (also `0`/`false`/`no`) creates no `wgpu` instance, requests no adapter and creates no device — nothing probes Vulkan/Metal, so a host whose driver loses the device mid-run cannot be taken down by the probe. Discovery is GPU-only, so it then reports the same permanent, non-retryable *unavailable* verdict a GPU-less host receives, and evolution continues on the CPU. `auto`/`on`/unset probe as before; an unrecognised value is logged and treated as `auto`. Set it **outside** the process (systemd unit, launch script) — the library never writes its own GPU variables. |
...
| `NEAT_AI_DISCOVERY_MIN_DISTINCT_TARGETS_PER_BATCH` | 3 | Minimum distinct target neurons in an emitted add-neuron batch when the candidate pool supports it. The top of the rank-sorted list is reordered to cover this many distinct targets before the per-target cap is applied (range 1–32) (Issue #1193). |
| `NEAT_AI_DISCOVERY_NEURON_RANKING_BANDS` | 10 | Number of reliability bands add-neuron candidates are ranked into (Issue #1924). Candidates are ordered by their improved-sample share (`improvedCount / totalCount`) bucketed into this many bands, with `expectedCreatureScoreGain` breaking ties **within** a band only — the cached corpus measured that estimate at r = −0.61 against success, so it can no longer lift a candidate past a more reliable one. This is the gain-versus-reliability knob: `1` collapses the order back to gain-descending (the pre-#1924 behaviour), larger values hand more of the ordering to reliability. Clamped to `[1, 1000]`. |
