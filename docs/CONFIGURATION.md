...
**Surrounding whitespace is always tolerated** on numeric variables (Issue
#2006) — `" 5 "` and `"5\n"` tune the knob exactly as `"5"` does, so a value
carried in from a shell heredoc or a YAML `env:` block is not silently dropped.
