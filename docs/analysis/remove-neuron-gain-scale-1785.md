...
| `REMOVE_LOW_IMPACT_NOISE_FLOOR` = `1e-5` (`candidate_scoring.rs:1469`) vs host `costOfGrowth` = `1e-7` | Gate 2: an absolute `f32` screening `boostedSavings − contribution`, a term **linear in `costOfGrowth`**; unreachable below 657 synapses on one neuron | Same category error as Gate 1's, on the other path. This document fixed only the **shared denomination — units of `costOfGrowth`**, the property that stops the two gates contradicting each other. **#1814 has landed**: the constant is now `REMOVE_LOW_IMPACT_NOISE_FLOOR_UNITS = 1.0` with `remove_low_impact_noise_floor(cost_of_growth)`, the #1142 numerical-noise guarantee is preserved (the `6.64e-8` class is `0.664` units, below the `1.0`-unit floor) and the `NEAT_AI_DISCOVERY_REMOVE_LOW_IMPACT_NOISE_FLOOR` override still pins an absolute value. | **Yes — #1814** |
...
| [#1814](https://github.com/stSoftwareAU/NEAT-AI-Discovery/issues/1814) | Gate 2, **landed**. Shares this document's denomination (units of `costOfGrowth`); `REMOVE_LOW_IMPACT_NOISE_FLOOR` is replaced by `REMOVE_LOW_IMPACT_NOISE_FLOOR_UNITS = 1.0` and `remove_low_impact_noise_floor(cost_of_growth)`. |
