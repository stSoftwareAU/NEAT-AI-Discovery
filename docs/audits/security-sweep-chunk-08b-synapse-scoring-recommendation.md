...
| the other nine `scoring` files | none | none of them allocates a collection with a size hint; the input-keyed `HashMap` growth in `calibration_correction.rs` has no size hint and is analysed in the outcome below. `error_distribution.rs::detect_modes_histogram` and its `vec![Vec::new(); NUM_BINS]` were removed by #2177 as the dead-lever cleanup — see the "Out-of-class observation" note below | n/a |
